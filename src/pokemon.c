...
static const u16 sRandomSpeciesBugBst[] =
{
  ...
  SPECIES_BURMY_PLANT,
  SPECIES_WIMPOD,
    // TODO: allow leaders to have shedinja
  // SPECIES_SHEDINJA,
  SPECIES_COMBEE,
  SPECIES_SPINARAK,
  SPECIES_VENIPEDE,
...
static u16 GetRandomSpeciesWithVariance(u16 species, u8 mapBased, u8 type, u16 additionalOffset, u16 maxVariance) //INTERNAL use only!
{
    // u8 slot, slotNew;
    ...
    if (gSaveBlock1Ptr->tx_Random_Similar)
    {
        ...
        if (speciesIndex < maxVariance) {
            modulo -= (maxVariance - speciesIndex);
        }
        if (speciesIndex > (numMons - maxVariance - 1)) {
            modulo -= (speciesIndex - (numMons - maxVariance - 1));
        }
        ...
        if (speciesIndex < maxVariance) {
            index = offset;
        } else if (speciesIndex > (numMons - maxVariance - 1)) {
            index = numMons - offset - 1;
        } else {
            index = speciesIndex + offset - maxVariance;
        }
    ...
    return sRandomSpeciesAllBst[RandomSeededModulo(species + mapOffset + additionalOffset, RANDOM_SPECIES_ALL_COUNT)];
}

static u16 GetRandomSpecies(u16 species, u8 mapBased, u8 type, u16 additionalOffset) //INTERNAL use only!
{
    return GetRandomSpeciesWithVariance(species, mapBased, type, additionalOffset, 100);
}
...
u16 GetSpeciesRandomSeededWithVariance(u16 species, u8 type, u16 additionalOffset, u16 maxVariance)
{
    // u8 slot, slotNew;
    u16 speciesResult = species;
    u8 mapBased = FALSE;
    ...
    case TX_RANDOM_T_STARTER_POKEMON:
        mapBased = gSaveBlock1Ptr->tx_Random_MapBased;
            speciesResult = GetRandomSpeciesWithVariance(species, mapBased, type, additionalOffset, maxVariance);
        break;
    case TX_RANDOM_T_WILD_POKEMON:
        mapBased = gSaveBlock1Ptr->tx_Random_MapBased;
...
{
    return GetSpeciesRandomSeededWithVariance(species, type, additionalOffset, 100);
}
